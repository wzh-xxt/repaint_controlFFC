...
def main(conf: conf_mgt.Default_Conf):
    ...
    checkpoint_path="/root/autodl-tmp/RePaint/data/pretrained/model_checkpoint3.pth"
    ...
    checkpoint_path = "/root/autodl-tmp/RePaint/data/pretrained/controlnet/control2.pth"
    control = FFCResNetGenerator()
    data_package = th.load(checkpoint_path, map_location="cpu")

    # ema_list = data_package["ema_params1"]
    # new_state_dict = {}
    # for i, (name, _value) in enumerate(control.named_parameters()):
    #     new_state_dict[name] = ema_list[i]
    ...
    # control.load_state_dict(new_state_dict)
    # # control.load_state_dict(data_package["model"])
    print("Successfully loaded EMA weights to control!")
