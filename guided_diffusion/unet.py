...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, outs, y=None, gt=None, **kwargs):
        """
        ...
        h = x.type(self.dtype)
        control_levels = [0, 5, 8, 11, 13,17]
        for level,module in enumerate(self.input_blocks):
            h = module(h, emb)
            if level in control_levels:
                idx = control_levels.index(level)
                h = h + outs[idx]
            hs.append(h)
        h = self.middle_block(h, emb)+outs[-1]
